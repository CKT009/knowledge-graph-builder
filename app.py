...
from workflows_new.langgraph_router import autonomus_pipeline
...
if topic:
    with st.spinner("🔎 Generating your detailed knowledge graph..."):
        graph = autonomus_pipeline(topic)
        svg_path = export_to_svg(graph, file_name=f"{topic}")
        st.success("✅ Graph generated successfully!")
        st.image(svg_path, caption=f"📌 Visual Knowledge Map for: {topic}",use_container_width =True)
